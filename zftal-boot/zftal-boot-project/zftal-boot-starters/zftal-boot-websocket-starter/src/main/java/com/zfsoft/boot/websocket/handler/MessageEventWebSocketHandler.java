package com.zfsoft.boot.websocket.handler;
...
import com.zfsoft.boot.websocket.session.handler.AbstractRouteableEventHandler;
import com.zfsoft.boot.websocket.session.handler.chain.HandlerChain;
import com.zfsoft.boot.websocket.session.handler.chain.HandlerChainResolver;
import com.zfsoft.boot.websocket.session.handler.chain.ProxiedHandlerChain;
import com.zfsoft.boot.websocket.utils.WebSocketUtils;

/**
 * 
 * @className	： MessageEventWebSocketHandler
 * @description	： 基于责任链模式的消息路径专门实现的Handler,该Handler会把消息分发给匹配的责任链实现
 * @author 		：万大龙（743）
 * @date		： 2017年12月4日 上午10:51:12
 * @version 	V1.0
 */
