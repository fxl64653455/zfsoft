 ...
 */
package com.zfsoft.boot.druid.util;
...
import com.zfsoft.boot.druid.ds.NewDruidDataSource;
import com.zfsoft.security.algorithm.DesBase64Codec;

public class DruidDataSourceUtils {

	private static DesBase64Codec dbencrypt = new DesBase64Codec();
	...
	public static DruidDataSource createDataSource(DataSourceProperties properties, DruidProperties druidProperties,
		...
		if(NewDruidDataSource.class.isAssignableFrom(properties.getType())) {
			/*
			// jdbcUrl: 连接数据库的url
			druidProperties.setUrl(dbencrypt.decrypt(jdbcUrl.getBytes()));
			// username: 连接数据库的用户名
			druidProperties.setUsername(dbencrypt.decrypt(username.getBytes()));
			// password: 连接数据库的密码
			druidProperties.setPassword(dbencrypt.decrypt(password.getBytes()));
			*/
		}
