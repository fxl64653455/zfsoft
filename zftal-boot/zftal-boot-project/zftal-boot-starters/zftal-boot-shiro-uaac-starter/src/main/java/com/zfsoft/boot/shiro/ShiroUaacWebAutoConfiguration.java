...
import com.zfsoft.authz.uaac.shiro.realm.UaacSsoRealm;
...
/**
 * 
 * @className	： ShiroUaacWebAutoConfiguration
 * @description	： uaac认证自动配置类
 * @author 		：魏广跃（1571）
...
public class ShiroUaacWebAutoConfiguration{
	...
	public Realm uaacSsoRealm(//
		...
		UaacSsoRealm realm = new UaacSsoRealm();
		...
		if(log.isDebugEnabled()) {
			log.debug("启用uaac认证，初始化uaac认证的Realm Bean["+ realm +"]");
		}
		return realm;
	}
}
