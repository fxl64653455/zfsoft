/**变更审核操作*/
function changeAudit(applyChangeId){
	$.showDialog(_path + '/manager/api/apply/change/new-audit/'+applyChangeId,'变更审核', $.extend({},addConfig,{
...
jQuery(function($){
	...
	$('#changeGrid').bootstrapTable({
		 url: _path + '/manager/api/apply/change/changeList',uniqueId: 'applyChangeId',
		 ...
		 columns: [{
	    	...
	    	title : '操作',formatter : function(value,row,index){
	    		...
	    		if(row.auditStatus == '0'){
	    			content += '<a style="padding:3px;color:#FFF;" class="zf-btn btn-primary" href="javascript:changeAudit(\''+row.applyChangeId+'\')">审核</a>&nbsp;&nbsp;';
	    		}
	    		content += '<a style="padding:3px;color:#FFF;" class="zf-btn btn-primary" href="'+_path + '/manager/api/apply/change/changeFile/'+row.applyChangeId+'">申请材料</a>&nbsp;&nbsp;';
	    		content += '<a style="padding:3px;color:#FFF;" class="zf-btn btn-primary" href="'+_path +'/manager/api/index/detail/'+row.applyFor+'" target="_blank">接口详情</a>';
	    		return content;
	    	}
	    }],
