...
function changeApply(applyId){
	var row = $('#applyGrid').getRow(applyId);
	$.showDialog(_path + '/manager/api/apply/change/new-change/' + row.applyFor + '/' + row.applyId,'变更申请',$.extend({},addConfig,{
...
jQuery(function($){
	...
	$('#applyGrid').loadGrid({
		 url: _path + '/manager/api/apply/applyData',uniqueId: 'applyId',
		 ...
		 columns: [{
	    	...
	    	title : '操作',formatter : function(value,row,index){
	    		...
	    		if(row.applyStatus == '1'){
	    			content += '&nbsp;&nbsp;<a style="padding:3px;color:#FFF;" class="zf-btn btn-primary" href="javascript:changeApply(\''+row.applyId+'\')">变更申请</a>';
	    		}
	    		if(authType == 'api' && row.applyStatus == '1'){
	    			content += '&nbsp;&nbsp;<a style="padding:3px;color:#FFF;" class="zf-btn btn-primary" href="javascript:showSecret(\''+row.apiKey+'\',\''+row.apiSecret+'\')">查看密钥</a>';
	    		}
	    		return content;
	    	}
	    }],
