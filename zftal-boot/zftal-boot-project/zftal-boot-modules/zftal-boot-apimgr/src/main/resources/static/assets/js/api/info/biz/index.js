jQuery(function($){
	...
	$('#tabGrid').loadGrid({
		 url			: _path + '/manager/api/info/biz/list',
		 uniqueId		: "id",
		 toolbar		: '#but_ancd',
		 sortName		: 'API_TIME',
		 sortOrder		: "desc",
	...
	$("#btn_zj").click(function () {
		$.showDialog(_path + '/manager/api/info/biz/to/new-biz-api','增加业务接口信息', $.extend({},modifyConfig,{
			max_height : "400px",
			//height: '600px',
		 	//是否使用模拟滚动条
	...
	$("#btn_xg").click(function () {
		...
		$.showDialog(_path + '/manager/api/info/biz/to/edit-biz-api?id='+ keys[0],'修改业务接口信息', $.extend({},modifyConfig,{
			width : '1000px',
			max_height : "400px",
			//是否使用模拟滚动条
			customScrollbar : true,btnlock : true
	...
	$("#btn_sc").click(function () {
		...
		$('#tabGrid').plcz(_path + '/manager/api/info/biz/delete?ids=' + keys.join(","),'删除业务接口信息');
	});
	...
	$("#btn_detail").click(function () {
		...
		//业务判断
		$.showDialog(_path + '/manager/api/info/biz/to/detail/'+ keys[0] ,'接口详情', $.extend({},viewConfig,{
			width : '1000px',
			max_height : "400px",
			//是否使用模拟滚动条
			customScrollbar : true,btnlock : true
	...
	$(document).off("click","a.label-deployed").on("click","a.label-deployed",function(event){
		var index = $(this).data("index");
		
		$('#tabGrid').bootstrapTable("uncheckAll");
    	$('#tabGrid').bootstrapTable("check",index);
