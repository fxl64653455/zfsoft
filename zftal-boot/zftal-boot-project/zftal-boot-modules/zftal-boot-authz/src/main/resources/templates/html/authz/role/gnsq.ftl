...
<html>
	<head>
		<style type="text/css">
			...
			.function_list02 li { float:left; background:url('${request.contextPath}/assets/images/fgx_06.gif') left center no-repeat; padding:0 10px; white-space:nowrap; }
	...
	<body>
		...
		<form id="ajaxForm" action="saveGnqx" method="post" data-toggle="validation" role="form" class="form-horizontal sl_all_form">
		...
		[#list buttonList as button]
			<input type="hidden" value="${button.gnczid}" name="userButton"/>
		[/#list]
		</form>
	</body>
	<script type="text/javascript" src="${request.contextPath}/assets/js/authz/role/jsgl_gnsq.js?ver=${messageUtil("zftal.jsVersion")}"></script>
	<script type="text/javascript">
		$(function(){
			$("#sjsq").click(function(){
				$("#modifyModal").reloadDialog({
					href:"sjsq?jsdm=${model.jsdm}",
				    onLoaded	:function(){
