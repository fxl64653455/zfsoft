jQuery(function($){
	...
	var options = {
			 ...
			 columns: [
	            ...
	            {field: 'lxmc',title: '类型',sortable:false,width:'350px'}, 
	            {field: 'dm',title: '代码',sortable:false,width:'250px'},
	            {field: 'mc',title: '名称',sortable:false, width:'300px'},
	            {field: 'xtjb',title: '级别',sortable:false,width:'100px',formatter:function(value,row,index){
	            	if(value == 'xt'){return '系统';}
	            	else if(value == 'yw'){return '业务';}
	            	return '';
	            }},
	            {field: 'zt',title: '启用状态',sortable:false,width:'100px',align:'center',formatter:function(value,row,index){
	            	if(value == '0'){
...
//	$("#tabGrid").loadJqGrid(JcsjGrid);
	...
	jQuery("#btn_zj").click(function () {
		$.showDialog("zjJcsj",'增加',addJCSJConfig);
	});
			...
			if (ids.length == 0){
				...
				$.confirm('您确定要删除选择的记录吗？',function(result){
					if(result){
						...
						jQuery.post("scJcsj",{"ids":ids.join(","), "lxs": lxs.join(","), "dms": dms.join(",")},function(responseText){
							setTimeout(function(){
								if(responseText.status == 'success'){
									$.success(responseText.message,function() {
										$('#tabGrid').refreshGrid();
	...
	jQuery("#btn_xg").click(function () {
		...
		$.showDialog("xgJcsj?pkValue="+row.pkValue,'修改',$.extend({},modifyConfig,{"width":500}));
	});
